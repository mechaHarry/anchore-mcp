"""Image-detail evidence adapter."""

from fastmcp import Context
from fastmcp.tools import ToolResult

from anchore_mcp.anchore.http import MAX_RESPONSE_BYTES
from anchore_mcp.anchore.routes import image_by_digest_route
...
from anchore_mcp.models.results import ImageDetailResult
...
async def anchore_image_detail(context: Context, locator: ImageLocator) -> ToolResult:
    try:
        ...
        response = await runtime.anchore_http.get_json(
            connection,
            image_by_digest_route(connection.api_version, selected.digest),
            max_response_bytes=MAX_RESPONSE_BYTES,
        )
        result = ImageDetailResult(
        ...
        return success_result("Fetched detail evidence for the selected image.", result)
