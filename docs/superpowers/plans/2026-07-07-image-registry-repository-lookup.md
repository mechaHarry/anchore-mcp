...
**Architecture:** Keep digest and exact tagged-reference selection unchanged at the MCP boundary. Resolve exact references through `GET /v2/images?full_tag=...`; resolve a registry/repository pair through the wire-faithful `GET /v2/summaries/image-tags?registry=...&repository=...`, then use the selected digest for downstream image routes. Make the MCP schema, safe error provenance, examples, and historical design record describe the breaking contract consistently.
...
- [ ] **Step 1: Write failing exact-component selection tests**
...
- [ ] **Step 3: Implement the minimal component locator and exact matching**
...
- [ ] **Step 1: Write failing validation and safe-error tests**
...
- [ ] **Step 3: Update the MCP schema and safe error mapping**
...
- [ ] **Step 5: Commit the public-contract enforcement**
...
### Task 3: Correct lookup routes to the Anchore v2 wire contract
...
- [ ] **Step 1: Write failing route and pagination tests**
  ...
  `/v2/images?full_tag=...`. Cover `items` plus `total_rows` pagination, cap
  exhaustion, exact local `full_tag` matching, and numeric epoch `analyzed_at`.
...
- [ ] **Step 2: Run focused tests to verify they fail**
  ...
  Expected: FAIL because repository selection still calls `/images` and exact
  reference resolution still emits the undocumented `fulltag` wire key.

- [ ] **Step 3: Implement the wire-faithful routes**
  ...
  `full_tag` on the wire. Remove `registry`, `repository`, and `repo` from the
  `/images` fallback allowlist; deployment OpenAPI may still add advertised
  custom parameters.
...
- [ ] **Step 1: Update consumer examples and contract wording**
  ...
  `anchore_list_images.fulltag` is translated to `full_tag` on the wire. Correct
  both design documents and API research so they do not imply registry or
  repository fallback filters on `/images`; the deployment `/v2/openapi.json`
...
- [ ] **Step 2: Run documentation reference scan**
  ...
  Run: `rg -n '/images\?fulltag|image_repository="[^" ]*/|image_repository\s*\([^)]*registry/repo|registry.*repository.*(/v2/)?images|/images.*registry.*repository' README.md AGENTS.md examples docs`

  Expected: no current documentation presents `image_repository` as a combined
  registry/repository value, emits `/images?fulltag`, or treats registry and
  repository as assumed `/images` filters. Historical text may name the former
  combined input only when explicitly describing it as removed or wrong.
