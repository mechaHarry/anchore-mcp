...
- `image_registry` and `image_repository` must be supplied together. They are separate components such as `registry.example.com` and `team/app`; the MCP selects the newest analyzed tag from the exact pair through the verified `GET /v2/summaries/image-tags?registry=...&repository=...` route. V1 uses `/v1/summaries/image-tags` only after `/v1/openapi.json` explicitly advertises a compatible prefixed or unprefixed `GET` operation with direct `registry` and `repository` query parameters; otherwise selection fails closed as unsupported without trying the route.
...
- For `image_reference`, query `/v2/images` with wire parameter `full_tag`, or `/v1/images` with legacy wire parameter `fulltag`, require an exact local tag match, then apply this tool’s newest-selection timestamp proof.
...
The selector does not fall back to assumed `registry`, `repository`, or `repo` filters on `/v2/images`. V2 summary routing is implemented directly; v1 summary routing requires the capability proof above. The public `anchore_list_images.fulltag` input is a separate convenience field: it becomes `full_tag` on v2 and remains `fulltag` on v1.

Newest image selection in `anchore_policy_blocking_vulnerabilities` requires reliable timestamp metadata from Anchore image records for both its exact-reference and registry/repository locators. Every exact matching candidate that has a digest must also have a reliable analysis timestamp (`analyzed_at` or an accepted equivalent). A missing, invalid, or untrusted timestamp on any such candidate makes selection fail closed; a matching row without a digest cannot be selected and may be ignored. A tie at the newest timestamp across different digests is also an MCP error rather than a guess. This rule does not apply to the shared exact-reference resolver used by other digest-keyed tools: that resolver does not choose newest or use timestamps, and instead requires exact local row evidence with explicit incomplete outcomes when bounded evidence inspection cannot finish.
