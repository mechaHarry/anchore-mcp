...
**Anchore connectivity and profiles**

- R3. Support **multiple named profiles** per MCP installation (for example prod, stage, lab), each associated with a single Anchore Enterprise deployment. Each profile **must** include an **HTTPS base URL** and credentials for **API token** authentication per **R12**. Profiles **may** include an optional **account name** (or equivalent) for **account demarcation**; whether it may be omitted and how Anchore behaves are specified in **R12**. No other authentication methods are in scope for v1 unless explicitly added later.
- R4. Provide **profile selection** with an **active default** profile and an **optional per-tool override** so a single install can switch context without editing files for every request.
...
- R11. The repository must be **self-sufficient for future maintainers and AI agents**: include clear guidance (for example `AGENTS.md` and a human-facing README) covering setup, configuration including profiles, safe handling of credentials, intended tool behavior, **operator context surfacing and elicitation** (R8), **PII: textual chat vs JSON previews vs files; IDE logs/history out of MCP scope** (R14), **export size** (R15), and **documented semantics** for the remediation handoff bundle so external skills can integrate reliably.

**Security and trust**

- R12. **Authentication to Anchore Enterprise** is **presumed** to be **HTTPS** to the configured base URL. For **API token** authentication, credentials follow the usual pattern where the **`password`** field holds the **API token** (from secure configuration, e.g. environment reference), and the **`username`** field must be set to the **literal string `_api_key`**—that literal is **not** a separate configuration key; it is the **required value** for `username` whenever the token is carried in `password`. Profiles **may** include optional **account name** (or equivalent) for **account demarcation**; whether that field must be non-empty, and how Anchore treats requests when it is unset, must be **validated** against Anchore Enterprise behavior during planning/implementation. Other auth schemes are out of scope for v1 unless explicitly added later.
...
- An SRE or automation job can run **repeatable fleet-style queries** against a chosen profile and produce **handoff payloads** suitable for **nightly** or periodic review plus remediation routing.
- Switching between **named profiles** works predictably via **default** and **override** behavior.
- Tool invocations surface enough **non-secret context** (profile, endpoint, account when set) to recognize mistakes (R8); **confirmation UX** is provided by the **host IDE** and **elicitation**, not a bespoke flow in this MCP.
...
- **Connectivity:** Multiple profiles per install; **HTTPS** with **`username` = literal `_api_key`** and **`password` = API token** (R12), optional **account name** for demarcation (R3, R12); **default profile + optional per-call override** (R4).
...
- [Affects R3, R11, R12][Technical] Storage format for **named profiles**; **`username`/`password`** with literal **`_api_key`** for username when password is the token; optional **account name**; whether **account name** may be omitted (Anchore behavior).
