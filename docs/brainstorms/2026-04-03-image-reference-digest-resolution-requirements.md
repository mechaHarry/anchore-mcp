...
- R3. **Resolution** must prefer **server-side narrowing** when the Anchore deployment supports it (e.g. `fulltag` on `GET /v2/images` as already used in `src/tools/images.ts`) before falling back to broader list strategies documented at planning time. Where resolution requires walking list results, **R10** applies so scans are not truncated at the first page.
...
| `docker.io/library/nginx:1.21` | Resolve via list/`fulltag` (or equivalent); then 0/1/N rules. |
...
- `anchore_list_images` with **`fulltag`** remains a valid primary resolution mechanism where supported.
...
- [Affects R3][Needs research] Whether any deployment supports additional query filters beyond `fulltag` / `vulnerability_id` that should participate in resolution for fewer false multi-matches; behavior when `fulltag` is ignored or partially applied (verify against OpenAPI; bounded fallback and failure mode).
