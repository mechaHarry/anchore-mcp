...
Anchore’s per-image HTTP routes use the **digest** in the path (`/v2/images/{digest}/…`). For convenience, digest-keyed tools also accept **`image_reference`**: a fully qualified tagged image string (`registry/repository:tag`, e.g. `docker.io/library/nginx:latest`). With the default v2 API, the MCP resolves the exact tag with `GET /v2/images?full_tag=…`; legacy v1 uses `GET /v1/images?fulltag=…`. It then makes the digest-keyed request. Short names like `nginx:latest` are rejected; use a registry-qualified reference.
...
- **Newest-image trust:** When exact-reference resolution must choose among matching rows, every exact matching digest-bearing candidate must have a reliable analysis timestamp (`analyzed_at` or an accepted equivalent). Missing, invalid, or untrusted timestamp data fails selection closed; matching digestless rows may be ignored.
