...
7. **Image lookup routes are identifier-specific:** Resolve an exact `image_reference` through **`GET /v2/images?full_tag=...`**. Resolve an `image_registry` + `image_repository` pair through **`GET /v2/summaries/image-tags?registry=...&repository=...`**, then choose the newest valid `analyzed_at`. Do not assume registry/repository fallback filters on `/images`; the deployment `/v2/openapi.json` is authoritative. See [docs/solutions/integration-issues/2026-07-08-anchore-v2-image-lookup-routes.md](docs/solutions/integration-issues/2026-07-08-anchore-v2-image-lookup-routes.md).
