...
7. **Image lookup routes are identifier-specific:** Shared exact `image_reference` resolution queries **`GET /v2/images?full_tag=...`** (legacy v1: **`GET /v1/images?fulltag=...`**) but trusts only bounded, exact reference evidence in returned rows; evidence overflow is `enumeration_incomplete`, not `no_match`, and timestamps are not used. `anchore_policy_blocking_vulnerabilities` calls verified **`GET /v2/summaries/image-tags?registry=...&repository=...`** directly for v2. It calls the v1 summary route only when same-origin, no-follow, bounded `/v1/openapi.json` capability evidence explicitly advertises compatible direct filters; otherwise it fails closed as unsupported. Only this policy tool’s newest-selection path applies timestamp trust. Do not assume registry/repository fallback filters on `/images`. See [docs/solutions/integration-issues/2026-07-08-anchore-v2-image-lookup-routes.md](docs/solutions/integration-issues/2026-07-08-anchore-v2-image-lookup-routes.md).
